...
use dlds\thepay\api\dataApi\TpDataApiResponseFactory;
...
class TpDataApiHelper {
    ...
    {
        $request = array_merge(
            array('accountId' => $config->accountId), (is_null($onlyActive) || $onlyActive === "") ? array() : array('onlyActive' => $onlyActive)
        );
    ...
    {
        $request = array_merge(
            array('merchantId' => $config->merchantId), $request
        );
