...
use dlds\thepay\api\dataApi\TpDataApiSignature;
...
class TpDataApiSignedArray {
    ...
    public static function createFromStdClass(\stdClass $value, $password = null, $arrayPaths = array())
    {
        return new TpDataApiSignedArray(
            self::stdClassToArrayRecursive($value), $password, $arrayPaths
        );
    }
    ...
    {
        return array_merge(
            $this->value, array('signature' => $this->signature())
        );
    }
    ...
    public function signature()
    {
        return TpDataApiSignature::compute(
                $this->value, $this->password, $this->arrayPaths);
    }
